<script lang="ts">
  ...
  $: availability = {
    ...
    numTimesAvailable:
      person.availability?.numTimesAvailable ??
      defaultAvailability.numTimesAvailable,
    numTimesSkip:
      person.availability?.numTimesSkip ?? defaultAvailability.numTimesSkip,
  };
...
<section class="card p-2 variant-ghost-surface relative overflow-auto">
  ...
  <div class="absolute top-2 right-2 action-buttons flex gap-2 items-center">
    ...
    <button
      ...
      on:click={() => $deletePersonMutation.mutate(person.id)}
    >
      <Trash />
    </button>
  </div>
